...
class TokenServiceImpl(TokenService):
    ...
    async def create_access_token(self, data: dict) -> str:
        ...
        access_token = jwt.encode(to_encode, self.__PRIVATE_KEY, algorithm=self.__ALGORITHM)
        return access_token
    ...
    async def create_refresh_token(self, data: dict) -> str:
        ...
        to_encode = data.copy()
        expire = datetime.now(timezone.utc) + timedelta(days=self.__REFRESH_TOKEN_EXPIRE_DAYS)
        jti = str(uuid.uuid4())
        to_encode.update({"exp": expire, "type": "refresh", "jti": jti})
        refresh_token = jwt.encode(to_encode, self.__PRIVATE_KEY, algorithm=self.__ALGORITHM)
